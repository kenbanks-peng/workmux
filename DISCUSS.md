...
- The Herdr adapter must not use Python. Replace the Python helper used for deferred cleanup with an implementation that does not require Python.

- The Herdr adapter should match Workmux's tmux behavior as closely as possible. Close / remove behavior currently differs:
  - **tmux:** Workmux closes the whole target window. This also closes any extra panes that you added to that window.
  - **Herdr:** Workmux closes only panes that it has verified as owned by Workmux. It does not close a whole tab or workspace.
