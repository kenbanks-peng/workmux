...
@pytest.mark.tmux_only
def test_open_multiple_uses_explicit_parent(
    mux_server: TmuxEnvironment, workmux_exe_path: Path, repo_path: Path
):
    write_workmux_config(repo_path, panes=[{}])
    for name in ("first", "last"):
        run_workmux_command(
            mux_server, workmux_exe_path, repo_path, f"add {name} --headless"
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "--target-name shared --parent-session parent",
            "--target-name cannot be used",
        ),
        ("--prompt hello --parent-session parent", "Prompt arguments"),
        ("--parent-session invalid:parent", "Parent session cannot contain ':'"),
    ],
)
def test_open_multiple_parent_keeps_other_option_checks(
    mux_server: MuxEnvironment, workmux_exe_path: Path, repo_path: Path, options, error
):
    result = run_workmux_command(
        mux_server,
        workmux_exe_path,
        repo_path,
        f"open first last {options}",
        expect_fail=True,
    )
