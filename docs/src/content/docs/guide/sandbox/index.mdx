...
but the agent can edit the Git repository. Those edits are inherently untrusted
and may run as your host user when you later build, test, or otherwise use the
repository. Workmux host Git operations can also run configured content filters
and merge drivers from the writable worktree.

Host-side hooks and pane commands from `.workmux.yaml` and explicitly proxied
host commands are also outside the sandbox boundary. Lima exposes writable Git
metadata.

**Do not use workmux sandbox if your threat model requires sandboxed agents to
have no influence over code that runs on the host.**
:::
...
pane. Git operations on the host honor configured content filters, merge
drivers, and credential helpers. Programs stored in the writable worktree remain
repository content and may change during a sandbox session. See [Git
hooks](/guide/sandbox/container/#git-hooks) for details.
