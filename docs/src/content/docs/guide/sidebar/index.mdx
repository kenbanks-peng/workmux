...
Stale agents are not jump targets: `{idx}` and `{jump_key}` skip them,
`workmux sidebar next|prev|jump <N>` passes over them, and the numbers close up
rather than leaving gaps. That holds wherever they are drawn, folded away or
sitting under a live agent, so what `Alt+1` reaches does not change with the
number of idle agents above it. Select them with `j`/`k` or click them instead.
