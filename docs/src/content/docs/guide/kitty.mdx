...
import screenshot from "../../../assets/screenshots/kitty-screenshot.webp";
...
<Screenshot
  src={screenshot}
  alt="workmux running in kitty with multiple agents"
  framed={false}
/>
...
| Pane sizing          | Percentage or cells  | Percentage with `splits` |
...
- **Pane sizing**: `percentage` uses Kitty's split bias with the `splits` layout. Absolute `size` values are unsupported because Kitty does not provide fixed-cell splits.
