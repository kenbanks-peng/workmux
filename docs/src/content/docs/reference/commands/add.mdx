...
| `--pr <number\|url>`           | Checkout a GitHub pull request or GitLab merge request into a worktree. Requires authenticated `gh` (GitHub) or `glab` 1.37.0 or newer (GitLab). Supports full URLs and custom local branch names, e.g., `workmux add review --pr 123`.                                 |
| `--forge <github\|gitlab>`     | Select the provider for `--pr`. Numeric references use GitLab when origin is on gitlab.com, otherwise GitHub. Use `--forge gitlab` for numeric references on self-hosted GitLab. Full URLs identify the provider.                                                       |
...
# Checkout a GitLab.com MR by number (origin points to gitlab.com)
workmux add --pr 123

# Checkout a self-hosted GitLab MR by number
workmux add --pr 123 --forge gitlab

# Checkout a self-hosted GitLab MR by URL, including nested groups
workmux add --pr https://gitlab.example.com/team/services/project/-/merge_requests/123
...
GitLab MR URLs must match the project that `glab` resolves from `origin`.
Workmux gives `glab` a disposable Git context containing only that remote, so
other remotes cannot select a different project. Repository/host environment
overrides and CI auto-login are disabled for these lookups. `glab` handles
authentication, SSH aliases, and configured SSH/API host mappings. Configure
self-hosted API protocols and ports in `glab`. The minimum supported version is
1.37.0; configured `ssh_host` mappings require 1.90.0 or newer.
Both GitLab and GitHub CLIs may be installed; provider selection does not depend
on which executable is available.

Same-project GitLab MRs use the source branch name. Fork MRs use
`gitlab-<source-project-id>-<source-branch>` by default, with a remote for the
actual source project. Pass a branch name to override this. Checkout first fetches
`refs/merge-requests/<number>/head` from origin, then explicitly fetches the source
branch if that ref is unavailable. The source project must be accessible; its
source branch may be deleted if the MR head ref remains available.

`--dry-run` reads MR metadata without adding remotes, fetching Git refs, or
creating worktrees. GitLab integration covers `add --pr`; PR status displays and
the dashboard's PR picker use GitHub.
